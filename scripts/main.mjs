...
import { runIPFSNetwork, stopIPFSNetwork, addFileToIPFS, getFileFromIPFS } from "./ipfs.mjs";
...
async function main() {
  ...
  const studentJSONData = JSON.stringify(students);
  const cidStudent = (await addFileToIPFS(studentJSONData)).toString();
...
export async function deployContract() {
  ...
  const contractFactory = await hre.ethers.getContractFactory("DistributedStorage");
